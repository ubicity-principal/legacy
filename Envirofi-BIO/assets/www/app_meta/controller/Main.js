...
var serverCouchURL = "@mdafapp.server.db.url@";

// in browser mode need to add username and passwort in order to establish proper replication!
if(phoneGapTarget == "browser") {
	serverCouchURL = 'http://'+ '@mdafapp.client.phonegap.username@'+ ':' + '@mdafapp.client.phonegap.password@' + '@' + '@mdafapp.client.phonegap.couchURL@';
}
...
function getCouchDBDatabasename() {
	if("@mdafapp.client.phonegap.dbname@" == "user"){
		activeDbName = "user";
		if(activeUser != null){
		...
		activeDbName = "@mdafapp.client.phonegap.dbname@";
		return activeDbName;
	}
		...
		function() {
			...
			nfc.write(
				...
				function(){
					...
					nfcListening = false;
                    Ext.Msg.alert('', '@treeapp.controller.Main.CreateTag.Success@');
				...
				function(){
                    ...
                    nfcListening = false;
                    Ext.Msg.alert('', '@treeapp.controller.Main.CreateTag.Fail@');
					//PROBLEM. NO RESULT AT REMOVING THE LISTENER
					nfc.removeNdefListener(
							function(){}, 
							function(){
...
function setUseCachedMaps(aoidoc, treeDetailsId) {
    ...
    if (dontShowAgain && dontShowAgain == '1') {
	    ...
	    var msg;
	    var msgPart1 = "@treeapp.controller.Main.useCachedMaps.Msg@<br/><br/><input type=\"radio\" id=\"local\" name=\"useCachedMaps\""; 
	    var msgPart2 = "/> @treeapp.controller.Main.useCachedMaps.Msg.Answer.Local@<br/><br/><input type=\"radio\" id=\"online\" name=\"useCachedMaps\""; 
	    var msgPart3 = '/> @treeapp.controller.Main.useCachedMaps.Msg.Answer.Online@<br/><br/><input type="checkbox" id="doInSettings" /> @treeapp.dialog.doInSettings@';
...
function NOT_addChannel(channelProperties) {
	...
	var dbInfo = userDB.getDoc("@mdafapp.client.phonegap.dbinfo@");
...
function popMainPanel() {
	...
	if (menuState == 'inMainNavigation' && Ext.getCmp('vtlmainnavigation').getPreviousItem()) {
		...
		// exit app
		Ext.Msg.confirm('', '@treeapp.dialog.terminateQuestion@', function(buttonId, value, opt) {
			if (buttonId == 'yes') {
...
function registerUser() {
    var dbInfo;
    //var userName = prompt("@treeapp.controller.Main.submitTreeObservation.noValidUserName@", "");
    ...
    while (userName == "") {
        ...
        userName = prompt("@treeapp.controller.Main.submitTreeObservation.noValidUserName@", "");
    }
    
    appSettings.userName = userName;
    
//  console.log ("@mdafapp.server.db.utils@" + "@mdafapp.server.db.utils.registerUser@" + "user=" + userName);
    var req = new XMLHttpRequest();

    req.open('GET', "@mdafapp.server.db.utils@" + "@mdafapp.server.db.utils.registerUser@" + "user=" + userName, false);
    req.send(null);
    if (req.readyState == 4) {
        ...
        if (dbInfo == null) {
        	...
        	if (dbInfo.docCount > 10) {
        		...
        		var loaderMessageBox = new Ext.MessageBox().show(
    				{
        				message: '@treeapp.controller.Main.Synchronizing@<br/><br/><progress id="progressBar" value="0" max="100"><span>0</span>%</progress>',
					...
					function (doc) {
	    				...
	    				if (newValue >= 100) {
	    					...
	    					var cachingMessageBox = new Ext.MessageBox().show(
    							{
    		        				message: '@treeapp.controller.Main.CachingMaps@',
    		          				buttons: Ext.MessageBox.OK,
    		        				fn: function(buttonId) {
        ...
        Ext.Msg.alert('', "@treeapp.controller.Main.NoInternetatStart@");
        // exit application
        navigator.app.exitApp();
        return;
    }
...
function hasInternetConnection(){
    if (typeof(device) !== 'undefined') {
        if(getCurrentNetworkState() != null){
           ...
           req.open('GET', "@mdafapp.test.url@", false);
           req.send(null);
           if (req.readyState == 4) {
...
function checkLogin(userEmail, pwd){
	if(hasInternetConnection()){
		...
		if(userEmail.length < 4){
			Ext.Msg.alert('', "@treeapp.view.Login.error.email@");
			return false;
		}
...
function registrationSuccess(userEmail){
	...
	Ext.getCmp('registrationSuccessTxt').setHtml("<h3>" + '@treeapp.view.Registration.registrationtext_1@' + ": " + userEmail + '@treeapp.view.Registration.registrationtext_2@' + "</h3><h4>" + '@treeapp.view.Registration.registrationtext_3@' + "</h4>");
...
function connectToCouchDB(couchURL) {
	...
	if("@mdafapp.client.phonegap.dbname@" == "user"){
		...
		userDB = new CouchJS(couchURL, activeDbName, "@mdafapp.client.phonegap.username@", "@mdafapp.client.phonegap.password@");
	} else {
		userDB = new CouchJS(couchURL, "@mdafapp.client.phonegap.dbname@", "@mdafapp.client.phonegap.username@", "@mdafapp.client.phonegap.password@");
	}
...
function finishInitialisation() {
	if(!userDB.exists()){
	...
	} else if (userDB.getDoc("@mdafapp.client.phonegap.dbinfo@") == null) {
		window.setTimeout(finishInitialisation,500);
	} else {
	    ...
	    if(restartConnectToCouchDB){
		    ...
		    if (dontShowAgain == null || dontShowAgain == '0') {
	            Ext.Msg.alert(
	                '',
	                '@treeapp.controller.Main.noCaching.Msg@<br/><br/><input type="checkbox" id="dontShowAgain" /> @treeapp.dialog.dontShowAgain@',
	                function() {
...
function getCouchDB() {
	if(!devReady){
		...
		if(phoneGapTarget == "phone") {
			...
			couchURL = 'http://'+ '@mdafapp.client.phonegap.username@'+ ':' + '@mdafapp.client.phonegap.password@' + '@' + '@mdafapp.client.phonegap.couchURL@';
			connectToCouchDB(couchURL);
			
		} else {
			Ext.Msg.alert('', '@treeapp.controller.Main.getCouchDB.Error@');
		}
...
function transferImageFile(serverURL, localFileName, successCallback, errorCallback) {
    ...
    var localFile = sdCardPath + "/" + "@mdafapp.client.imageCache@" + '/' + localFileName;
    ...
    window.requestFileSystem(
        ...
        function(fileSystem) {
            fileSystem.root.getDirectory(
                sdCardPath + "/" + "@mdafapp.client.imageCache@",
...
function loadImageFromServer(observation) {
    if (hasInternetConnection()) {
        Ext.Msg.confirm('', '@treeapp.view.TreeDetail.confirmImageDownload@', 
            function(buttonId) {
                if(buttonId === 'yes') {
                    ...
                    var imageDocURL = '@mdafapp.server.db.url@' + '@mdafapp.server.db.attachmentsDB@' + observation.imageDocId;
                            ...
                            if (req.status == 200) {
                                ...
                                transferImageFile(imageSrc, localFileName, 
                                    function (imageURI) {                                    
                                        ...
                                        removeOldFiles(sdCardPath + "/" + "@mdafapp.client.imageCache@", @mdafapp.client.imageCache.MaxEntries@);
                                    },
                                    function () {                                    
                                        // ToDo: download from couchDB failed => try download from cloud storage
                                    	Ext.Viewport.setMasked(false);
                                        Ext.Msg.alert('', '@treeapp.view.TreeDetail.errorLoadingImage@');
                                    }
                                );
                             }
                             else {
        ...
        Ext.Msg.alert('', '@treeapp.view.TreeDetail.errorLoadingImageConnection@');
    }
}
...
function loadImage(observationId) {
    ...
    if (observation.imageFile == undefined) {
        Ext.Msg.alert('', '@treeapp.view.TreeDetail.errorNoImage@');
        return;
    }
		...
		for (var i = 1; i < records.length + 1; i++) {
		    ...
		    if (records[i-1].getData().image != "null") {
			    ...
			    switch (records[i-1].getData().imagetype) {
                    case "TREE": records[i-1].getData().imagetype = "@treeapp.view.TreeObservation.ImageTypeTree@"; break;
                    case "LEAF": records[i-1].getData().imagetype = "@treeapp.view.TreeObservation.ImageTypeLeaf@"; break;
	                case "FRUIT": records[i-1].getData().imagetype = "@treeapp.view.TreeObservation.ImageTypeFruit@"; break;
	                case "BARK": records[i-1].getData().imagetype = "@treeapp.view.TreeObservation.ImageTypeBark@"; break;
	                case "FLOWER": records[i-1].getData().imagetype = "@treeapp.view.TreeObservation.ImageTypeFlower@"; break;
	                case "LEAFCONN": records[i-1].getData().imagetype = "@treeapp.view.TreeObservation.ImageTypeLeafConnection@"; break;
	                case "OTHER": records[i-1].getData().imagetype = "@treeapp.view.TreeObservation.ImageTypeOther@"; break;
	                default: break;
			    }
		    }
		    var idOoI;
...
function getCachingWarningText(){
	return '@treeapp.view.TreeMap.estimatedmem@';
}
...
function AoI_getDetailViewHtml(num) {
	...
	var label = '@treeapp.controller.Main.ObjectsInArea@';
...
Ext.define('treeapp.controller.Main', {
	...
	initApp: function(component, eOpts) {
		...
		Ext.apply(Ext.MessageBox, {YES: {text: '@treeapp.dialog.yesLabel@', itemId: 'yes', ui: 'action'}});
		Ext.apply(Ext.MessageBox, {NO: {text: '@treeapp.dialog.noLabel@', itemId: 'no', ui: 'normal'}});
		Ext.apply(Ext.MessageBox, {CANCEL: {text: '@treeapp.dialog.cancelLabel@', itemId: 'cancel'}});
	                                ...
	                                fileEntry.file(
                                		function(file) {
                                			...
                                			reader.onloadend = function(evt) {
                                			    ...
                                			    var dbInfo = userDB.getDoc("@mdafapp.client.phonegap.dbinfo@");
                                			    var dbName = userDB.getDoc("@mdafapp.client.phonegap.dbinfo@").dbname;
                                				var data = {
                                					db: dbName,
                            						log: fileContent,
                            						h: md5(dbName + "cy4ABbMap3mowzi5")
                                				}
                                				var url = "@mdafapp.server.db.utils@" + "@mdafapp.server.db.utils.uploadlog@";
                                				var response = Ext.Ajax.request({
                                				    url: url,
                                				    async : true,
                                				    success: function(response){
	...
	initializeTreeObservationData: function(component, eOpts) {
	    ...
	    if (observationInitialize.createTree == true) {
	        selectProperty.setOptions([
	                                   {text: '@treeapp.view.TreeDetail.SelectLabel@', value: 'empty'},
                                       ...
                                       {text: '@treeapp.view.TreeDetail.ClassificationPropertiesLabel@', value: 'classificationId'},
                                       {text: '@treeapp.view.TreeDetail.StatusPropertiesLabel@', value: 'statusId'}                                       
	                         ]);
 ...
 //               targetHeight: 2592,
	...
	submitTreeObservation: function(button, event, eOpts) {
	    ...
	    setObservedProperties(treeForm, currentTime.getTime(), sendString, function(success, counter) {
	        if (success) {
	            if (counter > 0) {
	                if (userDB.putDoc(sendString)) {
	                    Ext.Msg.alert('', @treeapp.controller.Main.submitTreeObservation.Success@);
	                } else {
	                    Ext.Msg.alert('', @treeapp.controller.Main.submitTreeObservation.Error@);
	                };
	            }
	            else {
	                Ext.Msg.alert('', @treeapp.controller.Main.submitTreeObservation.ErrorNoData@);
	            }
	            popMainPanel(); 
	            Ext.Viewport.setMasked(false);
	        }
	...
	submitNewTree: function(button, event, eOpts) {
		...
		if(!treeForm.getValues().treenumber || treeForm.getValues().treenumber == null || treeForm.getValues().treenumber == "") {
			Ext.Viewport.setMasked(false);
			Ext.Msg.alert('', @treeapp.controller.Main.submitNewTree.MandatoryFieldMissing@);
			return;
		}
...
//		verifyLocation(treeForm.getValues().location_lon, treeForm.getValues().location_lat);
		...
		setObservedProperties(treeForm, currentTime.getTime(), properties, function(success, counter) {
		    if (success) {
		        ...
		        if (userDB.putDoc(sendString)) {
		            Ext.Msg.alert('', @treeapp.controller.Main.submitNewTree.Success@);
		        } else {
		            Ext.Msg.alert('', @treeapp.controller.Main.submitTreeObservation.Error@);
		        };
		        popMainPanel(); 
...
*/        
	...
	applyFilter: function(button, event, eOpts) {
		...
		Ext.Msg.alert('', '@treeapp.controller.Main.applyFilter.Msg@');
	},
	...
	removeFilter: function() {
		...
		Ext.Msg.alert('', '@treeapp.controller.Main.removeFilter.Msg@');
	}
